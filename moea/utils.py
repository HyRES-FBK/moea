...
def parse_input(input_file: Union[str, Path]) -> dict:
    """
    Parse any input file in ``data``, ``distributions``, or ``costs`` of
    EnergyPLAN and return the data as a dictionary.
    """
    with open(input_file, 'r', encoding='ansi') as f:
        lines = f.readlines()
    # Remove newlines and '=' characters
    data = [ln.strip().replace('=', '') for ln in lines]
    # Remove lines with xxx
    data = [d for d in data if 'xxx' not in d]
    # Read data in chunks of two lines
    data = {data[i * 2]: data[i * 2 + 1] for i in range(len(data) // 2)}
    # Replace comma with dot
    data = {k: v.replace(',', '.') for k, v in data.items()}
    # Turn values into floats if they contain only numbers and a dot
    data = {k: float(v) if v.replace('.', '').isdigit() else v
            for k, v in data.items()}
    return data


def dump_input(input_dict: dict, i: int, data_file: Union[str, Path]) -> None:
    """
    ...
    data_file : str or Path
        The path to the data file generated by EnergyPLAN that defines the
